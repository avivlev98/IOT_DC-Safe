...
class MainWindow(QMainWindow):    
    def __init__(self, args, parent=None):
        ...
        if 'RACK' in self.name:
            tmp_upd = 22
            # Creating timer for update rate support
            self.timer = QtCore.QTimer(self)
            self.timer.timeout.connect(self.create_data_Rack)
            self.timer.start(int(self.update_rate)*1000)     
        elif 'ELECTRICITY' in self.name:
            tmp_upd = 22
            # Creating timer for update rate support
            self.timer = QtCore.QTimer(self)
            self.timer.timeout.connect(self.create_data_E)
            self.timer.start(int(self.update_rate)*1000) 
        elif 'WATER' in self.name: 
            # Creating timer for update rate support
            self.timer = QtCore.QTimer(self)
            self.timer.timeout.connect(self.create_data_W)
            self.timer.start(int(self.update_rate)*1000) # in msec        
    ...
    def create_data_Rack(self):
        global tmp_upd
        ic('Next update')
        temp=tmp_upd+random.randrange(1,10)
        current_data= 'From: ' + self.name+ ' Temperature_Rack: '+str(temp)
        self.connectionDock.Temperature.setText(str(temp))
        if not self.mc.connected:
            self.connectionDock.on_button_connect_click()
        self.mc.publish_to(self.topic_pub,current_data)

    def create_data_E(self):
        ic('Electricitydata update')
    ...
    def create_data_W(self):
        ic('Water data update')
        hour_delta_w = 0.42/24
        water=format(hour_delta_w +random.randrange(-10,10)/1000, '.3f')
        current_data= 'From: ' + self.name + ' Water: '+str(water)
